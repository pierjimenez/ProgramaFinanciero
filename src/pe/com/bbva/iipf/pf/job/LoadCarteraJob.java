...
@Service("loadCarteraJob")
public class LoadCarteraJob{
	Logger logger = Logger.getLogger(this.getClass());
	
	@Resource
	private LoadCarteraBO loadCarteraBO;
	...
	public void executeLoadMassive(){
		...
		try {
			logger.info("INICIO loadCartera JOB");
			Long idParametro=0L;
			 boolean TodoOk = true;
			...
			idParametro=parametroBO.ValidaCargaEnProceso(Constantes.COD_MONITORJOB_CARTERA);	//para disparar un Job una sola vez									
			if (idParametro>0) {
				
				int  contador=0;				
				contador=parametroBO.ValidaCargaAutomatica(stroFechaVerif,Constantes.ARCHIVO_CARTERA);
				
				if (contador >0){
					 TodoOk = false;					
				 }
				 if (TodoOk) {					
					 loadCarteraBO.executeLoadMassive(Constantes.TIPOCARGA_AUTOMATICO);
				 }
			updateParametroMonitorInactivo(Constantes.COD_MONITORJOB_CARTERA);
			}
			logger.info("FIN loadCartera JOB");
		} catch (Exception e) {
			updateParametroMonitorInactivo(Constantes.COD_MONITORJOB_CARTERA);
	...
	public void executeLoadManual(){
		...
		try {
			logger.info("INICIO loadCartera JOB");					
			loadCarteraBO.executeLoadMassive(Constantes.TIPOCARGA_MANUAL);	
			logger.info("FIN loadCartera JOB");
		} catch (Exception e) {			
			logger.error(e.getMessage(), e);
		}
