...
@Service("loadRatingJob")
public class LoadRatingJob{
	Logger logger = Logger.getLogger(this.getClass());
	
	@Resource
	private LoadRatingBO loadRatingBO;
	...
	public void executeLoadMassive(){
		...
		try {
			logger.info("INICIO loadRating JOB");
			Long idParametro=0L;
			 boolean TodoOk = true;
			...
			idParametro=parametroBO.ValidaCargaEnProceso(Constantes.COD_MONITORJOB_RATING);	//para disparar un Job una sola vez									
			if (idParametro>0) {
				int  contador=0;				
				contador=parametroBO.ValidaCargaAutomatica(stroFechaVerif,Constantes.ARCHIVO_RATING);
				
				if (contador >0){
					 TodoOk = false;					
				 }
				 if (TodoOk) {	
					 loadRatingBO.executeLoadMassive(Constantes.TIPOCARGA_AUTOMATICO);
				 }

			updateParametroMonitorInactivo(Constantes.COD_MONITORJOB_RATING);
			}
			logger.info("FIN loadRating JOB");
		} catch (Exception e) {
			updateParametroMonitorInactivo(Constantes.COD_MONITORJOB_RATING);
	...
	public void executeLoadManual(){
		...
		try {
			logger.info("INICIO loadRating JOB");		
			loadRatingBO.executeLoadMassive(Constantes.TIPOCARGA_MANUAL);			
			logger.info("FIN loadRating JOB");
		} catch (Exception e) {			
			logger.error(e.getMessage(), e);
		}
