 ...
 * se reemplazo FCKTools.RegisterDollarFunction=function(A){A.$=A.document.getElementById;};  
 * por FCKTools.RegisterDollarFunction=function(A){A.$=function(v){return A.document.getElementById(v);}};
 */
...
function Doc_OnMouseDown(A) {
	var e = A.srcElement;
	if (e.nodeName && e.nodeName.IEquals('input')
			&& e.type.IEquals([ 'radio', 'checkbox' ]) && !e.disabled) {
		e.disabled = true;
...
FCKTools.RegisterDollarFunction = function(A) {
	A.$ = function(v) {
		return A.document.getElementById(v);
	}
};
...
(function() {
	var A = [];
	var B = (FCK && FCK.EditorWindow && FCK.EditorWindow.parent.parent.scayt) ? true
			: false;
	var C = false;
	var D = false;
		...
		case 'About':
			...
			if (B && D)
				FCKDialog.OpenDialog('Scayt', 'SCAYT Settings',
						'dialog/fck_scayt.html?' + c.toLowerCase(), 343, 343);
			break;
		default:
			if (!B) {
				var U = this;
	...
	Z.prototype.GetLabel = function() {
		...
		return FCKBrowserInfo.IsIE ? '<div class="TB_Button_Image"><img src="'
				+ a + '" style="top:-192px"></div>'
